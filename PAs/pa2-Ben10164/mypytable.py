...
#
# Note: i used https://stackoverflow.com/questions/25023955/python-csv-writer-how-to-handle-quotes-in-order-to-avoid-triple-quotes-in-outpu
# for the csv writer formatting because i ran into problems with it outputting multiple double quotes in a row.
##############################################
import copy
import csv
...
class MyPyTable:
    ...
    def get_column(self, col_identifier, include_missing_values=True):
        ...
        """
        if col_identifier.isnumeric():  # check to see if the col_identifier is an int (the index) or the string (the label)
        ...
        for row in self.data:
            if(include_missing_values):  # check to see if the include_missing_values flag is active
    ...
    def save_to_file(self, filename):
        ...
        writer = csv.writer(outfile, delimiter=',',
                            quotechar='\'', quoting=csv.QUOTE_MINIMAL)  # found this on stackoverflow
    ...
    def find_duplicates(self, key_column_names):
        ...
        for i in range(len(self.data)):
            ...
            for col_name in key_column_names:
                ...
                temp_1D.append(self.data[i][index])

            if temp_1D in occured_already:  # if the temporary list that is only the key_column_names is in the list, it is a dupe
                dupe_list.append(i)
            else:
                occured_already.append(temp_1D)

        return dupe_list

    def remove_rows_with_missing_values(self):
        """Remove rows from the table data that contain a missing value ("NA")."""
        new_data = []  # a new list that self.data will be set to after the rest of the function
        ...
        for row in self.data:
            ...
            if row_clean:  # if it checked every value in the row and the flag is still true...
    ...
    def compute_summary_statistics(self, col_names):
        ...
        for col in col_names:  # this is going to go through each of the passed in names
            ...
            if(len(new_col) % 2 == 0):  # it is even
                new_median = (new_col[(len(new_col)-1) // 2] +
                              new_col[(len(new_col) // 2)]) / 2
            else:
        ...
        for row in range(len(self.data)):  # for every row in self.data
            ...
            for other_row in range(len(other_table.data)):
                ...
                for i in range(len(key_column_names)):
                    current_self_key_col_value = self.data[row][self.column_names.index(
                        key_column_names[i])]
                    current_other_key_col_value = other_table.data[other_row][other_table.column_names.index(
                        key_column_names[i])]
                ...
                if valid_row:
                    new_row = []
                    for col_name in new_col_names:  # we are going through each of the columns that will be in the resulting table
                        if(col_name in self.column_names):  # if the col_name is in self.column_names
                            new_row.append(
                                self.data[row][self.column_names.index(col_name)])  # we add the row to the new_row
                        else:  # otherwise it must be a other_table column exlusive
                            new_row.append(
                                other_table.data[other_row][other_table.column_names.index(col_name)])  # so we add it
                        ...
                        for new_col_name in new_col_names:
                            if(new_col_name in other_table.column_names):
                                if new_row[new_col_names.index(new_col_name)] != "NA":
                                    if(new_row[new_col_names.index(new_col_name)] != row[other_table.column_names.index(new_col_name)]):
                                        valid_row = False
                                        break
                        if valid_row:
                            newest_row = new_row.copy()
                            newest_row[new_col_names.index(
                                col_name)] = row[other_table.column_names.index(col_name)]
