    ...
    for index_list in index_lists:
        for i in range(folds):
            index_lists_folds[i].append(
                index_list[
                    i
                    * (len(index_list) // folds): (i + 1)
                    * (len(index_list) // folds)
                ]
            )
    ...
    for index_list in index_lists:
        for i in range(len(index_lists[0])):
            index_lists_folds[i].append(
                index_list[
                    i
                    * (len(index_list) // len(index_lists)): (i + 1)
                    * (len(index_list) // len(index_lists))
                ]
            )
    X_folds = []
    y_folds = []
...
def p_list(X_train, new_instance, y_train, value):
    ...
    for i in range(len(new_instance)):
        ...
        perdiction_list.append(attribute_matches_count / y_train.count(value))
    ...
    for att_value, att_partition in partitions.items():  # dictionary
        ...
        # CASE 3: no more instances to partition (empty partition) => backtrack and replace attribute node with majority vote leaf node
            ...
            value_subtree.append(
                [
                    "Leaf",
                    majority_vote(current_instances),
                    len(current_instances),
                    len(current_instances),
                ]
            )
...
def prepend_attribute_label(table, header=None):
    if header is None:
        for row in table:
            for i in range(len(row)):
                row[i] = "Att" + str(i) + " = " + row[i]
    else:
        for row in table:
            for i in range(len(row)):
...
def isNonMarketDataset(dataset):
    # TODO: this

    return False


def apriori(X_train, minsup, minconf):
    # TODO: this
    rules = []
    # step 1: Create L1 = set of supported itemsets of cardinality one
    L = []
    L.append([])  # this will be Lk-1
    for row in X_train:
        for item in row:
            if item not in L[0]:
                L[0].append(item)
    ...
    k = 2
    Lk_1 = L[0]
    # step 3: while Lk-1 is not empty
    while len(Lk_1) > 0:
        Ck = []

        # joins tep, compare each member of lk-1 with every other member.
        # if the first k - 2 members are the same, place A u B into Ck
        for i in range(len(Lk_1)):  # for each itemset in Lk-1
            for j in range(i + 1, len(Lk_1)):  # for each pair of itemsets
                # if the first k - 2 members are the same
                if Lk_1[i][: k - 2] == Lk_1[j][: k - 2]:
                    Ck.append(Lk_1[i] + Lk_1[j][k - 2:])  # append A u B to Ck

        # prune step, for each member c of Ck in turn{
        # examine all subsets of c with k - 1 elements
        # delete c from Ck if any of the subsets is not a member of Lk-1
        # }
        for c in Ck:  # for each itemset in Ck
            for i in range(len(c)):  # for each subset of c
                if c[:i] + c[i + 1:] not in Lk_1:  # if the subset is not in Lk-1
                    Ck.remove(c)  # remove c from Ck
                    break  # break out of the for loop
        # step 5:   prune all the itemsets in Ck that are not supported, to creat Lk
        Lk = []
        for c in Ck:  # for each itemset in Ck
            Nleft, Nright, Nboth, Ntotal = compute_rule_counts(
                {"lhs": c[: k - 2], "rhs": c[k - 2:]}, X_train
            )
            if Nboth / Ntotal >= minsup:
                Lk.append(c)
        # step 6:   add Lk to Lk-1
        L.append(Lk)
        rules = Lk_1
        Lk_1 = Lk
        k += 1
    # now that we have all the L's we can make the rules
    supported_itemsets = []
    for Lk in L:
        for c in Lk:
            supported_itemsets.append(c)
    rules = generate_apriori_rules(supported_itemsets, X_train, minconf)
    return rules
...
def generate_apriori_rules(supported_itemsets, table, minconf):
    ...
    for S in supported_itemsets:
        ...
        for i in range(len(S)):
            if S[i] not in RHSs and S[i + 1:] not in RHSs:
                if S[i + 1:] not in [""]:
                    RHSs.append(S[i])
                    total_RHSs.append(S[i])
                    LHSs.append(S[i + 1:])
                    total_LHSs.append(S[i + 1:])
    # check confidence >= minconf => append to rules
    LHSs = total_LHSs
    RHSs = total_RHSs
    for i in range(len(LHSs)):
        Nleft, Nright, Nboth, Ntotal = compute_rule_counts(
            {"lhs": LHSs[i], "rhs": RHSs[i]}, table
        )
...
def compute_confidence(LHS, RHS, table):
    Nleft, Nright, Nboth, Ntotal = compute_rule_counts(
        {"lhs": LHS, "rhs": RHS}, table)
    confidence = Nboth / Nleft
    support = Nboth / Ntotal
