...
class MyPyTable:
    ...
    def get_column(self, col_identifier, include_missing_values=True):
        ...
        if (
            ...
            try:
                ...
                raise ValueError("Invalid column identifier: " + col_identifier)
    ...
    def compute_summary_statistics(self, col_names):
        ...
        for col in col_names:  # this is going to go through each of the passed in names
            ...
            if len(new_col) % 2 == 0:  # it is even
                new_median = (
                    new_col[(len(new_col) - 1) // 2] + new_col[(len(new_col) // 2)]
                ) / 2
            else:
    ...
    def perform_full_outer_join(self, other_table, key_column_names):
        ...
        for row in range(len(self.data)):  # for every row in self.data
            ...
            for col_name in new_col_names:
                if (
                    col_name in self.column_names
                ):  # if the col_name is in self.column_names
                    # we add the row to the new_row
                    new_row.append(self.data[row][self.column_names.index(col_name)])
                else:  # otherwise it must be a other_table column exlusive
                    ...
                    for row2 in other_table.data:
                        ...
                        for new_col_name in new_col_names:
                            if new_col_name in other_table.column_names:
                                if new_row[new_col_names.index(new_col_name)] != "NA":
                                    if (
                                        new_row[new_col_names.index(new_col_name)]
                                        != row2[
                                            other_table.column_names.index(new_col_name)
                                        ]
                                    ):
                                        valid_row = False
                                        break
                        if valid_row:
                            ...
                            newest_row[new_col_names.index(col_name)] = row2[
                                other_table.column_names.index(col_name)
                            ]
        ...
        for i in range(len(other_table.data)):
            ...
            for j in range(len(new_data)):
                ...
                for k in range(len(key_column_names)):
                    current_self_key_col_value = other_table.data[i][
                        other_table.column_names.index(key_column_names[k])
                    ]
                    current_other_key_col_value = new_data[j][
                        new_col_names.index(key_column_names[k])
                    ]
                    if current_self_key_col_value != current_other_key_col_value:
                        valid_row = False
                        break
